...
export const GET = withApiHandler(async (req: NextRequest): Promise<PaginatedResponse<KanjiSummary>> => {
  ...
  const filtered = normalizedQuery ? rows.filter((row) => matchesQuery(row, normalizedQuery)) : rows;
