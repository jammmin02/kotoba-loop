...
import { WordDetailNav, WordDetailNavButtons } from "@/components/vocabulary/word-detail-nav";
...
export default async function WordDetailPage(props: PageProps<"/words/[id]">) {
  ...
  // 넷 다 서로 독립적인 조회라 병렬로 보내 왕복 지연을 한 번만 문다(DB가 다른 리전이라 특히 체감됨).
  const [userVocabulary, vocabulary, userSentence, bookWordIds] = await Promise.all([
    ...
    // bookId가 있으면 이전/다음 단어 이동에 쓸 순서를 가져온다. 다른 사용자의 단어장이면
    // book.user_id로 걸러져 빈 배열이 되고, 이전/다음 버튼은 자연히 비활성화된다.
    bookId
      ? db.vocabulary.findMany({
          where: {
            bookItems: { some: { vocabulary_book_id: bookId, book: { user_id: session.user.id } } },
          },
  ...
  const currentIndex = bookWordIds.findIndex((word) => word.id === id);
  const prevWordId = currentIndex > 0 ? bookWordIds[currentIndex - 1]?.id : undefined;
  const nextWordId =
    currentIndex >= 0 && currentIndex < bookWordIds.length - 1
      ? bookWordIds[currentIndex + 1]?.id
      : undefined;
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-2xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8">
      ...
      {bookId && (
        <div className="flex items-center justify-between gap-2">
          <WordDetailNavButtons prevHref={prevHref} nextHref={nextHref} />
          <Link href={`/study/custom?bookId=${bookId}`}>
            <Button type="button" variant="quest" size="sm">
              복습하기
            </Button>
          </Link>
        </div>
      ...
      <WordDetailNav prevHref={prevHref} nextHref={nextHref} className="flex flex-col gap-6">
